input = """Button A: X+62, Y+54
Button B: X+19, Y+93
Prize: X=4094, Y=11058

Button A: X+55, Y+93
Button B: X+83, Y+27
Prize: X=9995, Y=7833

Button A: X+83, Y+31
Button B: X+21, Y+34
Prize: X=8042, Y=4233

Button A: X+21, Y+11
Button B: X+11, Y+23
Prize: X=13963, Y=17749

Button A: X+65, Y+97
Button B: X+83, Y+12
Prize: X=6919, Y=3278

Button A: X+28, Y+17
Button B: X+16, Y+43
Prize: X=19828, Y=14869

Button A: X+26, Y+12
Button B: X+40, Y+59
Prize: X=19660, Y=1040

Button A: X+17, Y+80
Button B: X+77, Y+16
Prize: X=11125, Y=19344

Button A: X+41, Y+77
Button B: X+97, Y+15
Prize: X=6430, Y=4386

Button A: X+18, Y+58
Button B: X+68, Y+24
Prize: X=3136, Y=3276

Button A: X+40, Y+39
Button B: X+21, Y+98
Prize: X=2893, Y=8480

Button A: X+57, Y+11
Button B: X+42, Y+99
Prize: X=7356, Y=6237

Button A: X+21, Y+61
Button B: X+72, Y+24
Prize: X=7049, Y=5889

Button A: X+55, Y+21
Button B: X+20, Y+37
Prize: X=17600, Y=18892

Button A: X+39, Y+79
Button B: X+51, Y+14
Prize: X=11165, Y=15493

Button A: X+59, Y+97
Button B: X+35, Y+19
Prize: X=1331, Y=1957

Button A: X+65, Y+21
Button B: X+23, Y+54
Prize: X=8456, Y=3041

Button A: X+29, Y+52
Button B: X+42, Y+17
Prize: X=7340, Y=17904

Button A: X+97, Y+15
Button B: X+78, Y+75
Prize: X=10291, Y=6060

Button A: X+14, Y+48
Button B: X+49, Y+26
Prize: X=18015, Y=4490

Button A: X+47, Y+16
Button B: X+38, Y+68
Prize: X=4834, Y=6688

Button A: X+76, Y+19
Button B: X+16, Y+62
Prize: X=18040, Y=6100

Button A: X+15, Y+43
Button B: X+44, Y+15
Prize: X=18354, Y=6234

Button A: X+27, Y+73
Button B: X+63, Y+20
Prize: X=18080, Y=2029

Button A: X+52, Y+78
Button B: X+73, Y+31
Prize: X=6369, Y=3823

Button A: X+16, Y+55
Button B: X+33, Y+16
Prize: X=19647, Y=4165

Button A: X+23, Y+17
Button B: X+37, Y+86
Prize: X=2569, Y=3893

Button A: X+18, Y+57
Button B: X+70, Y+17
Prize: X=10998, Y=4825

Button A: X+16, Y+31
Button B: X+61, Y+36
Prize: X=262, Y=11902

Button A: X+48, Y+27
Button B: X+13, Y+43
Prize: X=6877, Y=7117

Button A: X+13, Y+36
Button B: X+49, Y+27
Prize: X=5930, Y=12113

Button A: X+62, Y+21
Button B: X+31, Y+76
Prize: X=3184, Y=1310

Button A: X+55, Y+13
Button B: X+33, Y+83
Prize: X=11298, Y=1134

Button A: X+68, Y+13
Button B: X+63, Y+92
Prize: X=2972, Y=888

Button A: X+18, Y+24
Button B: X+71, Y+11
Prize: X=6903, Y=2427

Button A: X+84, Y+17
Button B: X+40, Y+53
Prize: X=4860, Y=1253

Button A: X+74, Y+39
Button B: X+17, Y+51
Prize: X=5051, Y=6797

Button A: X+65, Y+18
Button B: X+24, Y+76
Prize: X=9457, Y=8214

Button A: X+66, Y+22
Button B: X+14, Y+43
Prize: X=810, Y=14750

Button A: X+12, Y+82
Button B: X+93, Y+70
Prize: X=3672, Y=9258

Button A: X+63, Y+26
Button B: X+35, Y+56
Prize: X=3948, Y=3998

Button A: X+51, Y+25
Button B: X+28, Y+54
Prize: X=3832, Y=4456

Button A: X+24, Y+62
Button B: X+67, Y+33
Prize: X=14082, Y=8066

Button A: X+33, Y+15
Button B: X+28, Y+54
Prize: X=4793, Y=1049

Button A: X+58, Y+25
Button B: X+27, Y+60
Prize: X=5684, Y=3440

Button A: X+51, Y+27
Button B: X+12, Y+33
Prize: X=6641, Y=12200

Button A: X+91, Y+41
Button B: X+39, Y+93
Prize: X=9633, Y=11355

Button A: X+16, Y+41
Button B: X+60, Y+19
Prize: X=11624, Y=17434

Button A: X+23, Y+62
Button B: X+70, Y+33
Prize: X=6285, Y=5378

Button A: X+63, Y+38
Button B: X+24, Y+52
Prize: X=18713, Y=4086

Button A: X+92, Y+17
Button B: X+44, Y+96
Prize: X=6932, Y=9980

Button A: X+92, Y+99
Button B: X+99, Y+17
Prize: X=9724, Y=2585

Button A: X+95, Y+16
Button B: X+63, Y+62
Prize: X=11570, Y=5032

Button A: X+49, Y+18
Button B: X+18, Y+28
Prize: X=5064, Y=2288

Button A: X+35, Y+77
Button B: X+53, Y+14
Prize: X=16536, Y=13857

Button A: X+14, Y+41
Button B: X+89, Y+71
Prize: X=8902, Y=8623

Button A: X+49, Y+17
Button B: X+27, Y+70
Prize: X=18325, Y=2157

Button A: X+20, Y+57
Button B: X+54, Y+18
Prize: X=19442, Y=3419

Button A: X+11, Y+25
Button B: X+72, Y+42
Prize: X=8781, Y=6799

Button A: X+20, Y+48
Button B: X+61, Y+37
Prize: X=10609, Y=5109

Button A: X+19, Y+45
Button B: X+42, Y+12
Prize: X=253, Y=1835

Button A: X+15, Y+48
Button B: X+64, Y+16
Prize: X=17517, Y=13728

Button A: X+52, Y+13
Button B: X+25, Y+66
Prize: X=8798, Y=10188

Button A: X+20, Y+46
Button B: X+51, Y+30
Prize: X=12086, Y=9494

Button A: X+21, Y+58
Button B: X+63, Y+22
Prize: X=17063, Y=1590

Button A: X+11, Y+39
Button B: X+75, Y+30
Prize: X=2732, Y=8453

Button A: X+57, Y+26
Button B: X+26, Y+47
Prize: X=16605, Y=523

Button A: X+64, Y+21
Button B: X+18, Y+80
Prize: X=2504, Y=5860

Button A: X+83, Y+15
Button B: X+56, Y+57
Prize: X=4586, Y=4251

Button A: X+33, Y+13
Button B: X+27, Y+58
Prize: X=12359, Y=17846

Button A: X+19, Y+39
Button B: X+59, Y+19
Prize: X=7183, Y=19763

Button A: X+17, Y+11
Button B: X+22, Y+49
Prize: X=9203, Y=17789

Button A: X+28, Y+99
Button B: X+87, Y+11
Prize: X=2165, Y=6765

Button A: X+69, Y+28
Button B: X+52, Y+70
Prize: X=10406, Y=8428

Button A: X+30, Y+12
Button B: X+53, Y+76
Prize: X=13030, Y=14808

Button A: X+17, Y+49
Button B: X+47, Y+28
Prize: X=9223, Y=9615

Button A: X+33, Y+34
Button B: X+76, Y+15
Prize: X=2661, Y=1982

Button A: X+19, Y+50
Button B: X+44, Y+15
Prize: X=8425, Y=17470

Button A: X+80, Y+45
Button B: X+17, Y+65
Prize: X=3863, Y=4335

Button A: X+27, Y+16
Button B: X+14, Y+28
Prize: X=8861, Y=17640

Button A: X+32, Y+91
Button B: X+61, Y+52
Prize: X=5445, Y=10504

Button A: X+13, Y+42
Button B: X+58, Y+38
Prize: X=6888, Y=14140

Button A: X+25, Y+61
Button B: X+40, Y+11
Prize: X=1375, Y=16891

Button A: X+11, Y+52
Button B: X+75, Y+29
Prize: X=4368, Y=12074

Button A: X+22, Y+53
Button B: X+56, Y+29
Prize: X=7670, Y=8250

Button A: X+32, Y+11
Button B: X+32, Y+68
Prize: X=7360, Y=16987

Button A: X+18, Y+46
Button B: X+33, Y+16
Prize: X=3564, Y=2548

Button A: X+11, Y+46
Button B: X+54, Y+32
Prize: X=10843, Y=10358

Button A: X+60, Y+83
Button B: X+32, Y+13
Prize: X=16748, Y=18281

Button A: X+32, Y+59
Button B: X+97, Y+19
Prize: X=6823, Y=6346

Button A: X+96, Y+60
Button B: X+47, Y+94
Prize: X=9828, Y=10020

Button A: X+12, Y+72
Button B: X+81, Y+86
Prize: X=3333, Y=8398

Button A: X+59, Y+30
Button B: X+54, Y+97
Prize: X=7038, Y=5804

Button A: X+21, Y+45
Button B: X+48, Y+27
Prize: X=12056, Y=269

Button A: X+80, Y+31
Button B: X+30, Y+82
Prize: X=4020, Y=5921

Button A: X+80, Y+35
Button B: X+25, Y+55
Prize: X=6620, Y=4130

Button A: X+31, Y+18
Button B: X+13, Y+31
Prize: X=15898, Y=6859

Button A: X+80, Y+30
Button B: X+20, Y+43
Prize: X=5820, Y=3283

Button A: X+13, Y+27
Button B: X+34, Y+15
Prize: X=17252, Y=377

Button A: X+49, Y+11
Button B: X+40, Y+76
Prize: X=16449, Y=15823

Button A: X+92, Y+32
Button B: X+28, Y+48
Prize: X=7096, Y=2736

Button A: X+22, Y+76
Button B: X+54, Y+13
Prize: X=19030, Y=18217

Button A: X+98, Y+24
Button B: X+56, Y+62
Prize: X=1162, Y=864

Button A: X+32, Y+80
Button B: X+72, Y+45
Prize: X=4448, Y=7340

Button A: X+45, Y+16
Button B: X+14, Y+27
Prize: X=13215, Y=3378

Button A: X+13, Y+25
Button B: X+34, Y+19
Prize: X=8660, Y=3962

Button A: X+78, Y+32
Button B: X+12, Y+61
Prize: X=14312, Y=6693

Button A: X+17, Y+72
Button B: X+53, Y+53
Prize: X=1650, Y=5445

Button A: X+99, Y+51
Button B: X+12, Y+53
Prize: X=10587, Y=9293

Button A: X+19, Y+62
Button B: X+72, Y+52
Prize: X=6359, Y=8310

Button A: X+75, Y+31
Button B: X+13, Y+40
Prize: X=8828, Y=551

Button A: X+64, Y+14
Button B: X+26, Y+42
Prize: X=6138, Y=3122

Button A: X+23, Y+89
Button B: X+92, Y+15
Prize: X=10143, Y=5490

Button A: X+19, Y+45
Button B: X+71, Y+52
Prize: X=1728, Y=1421

Button A: X+72, Y+19
Button B: X+32, Y+62
Prize: X=8328, Y=4929

Button A: X+15, Y+77
Button B: X+44, Y+11
Prize: X=16875, Y=8812

Button A: X+13, Y+66
Button B: X+82, Y+31
Prize: X=10424, Y=874

Button A: X+53, Y+27
Button B: X+36, Y+58
Prize: X=1610, Y=2010

Button A: X+16, Y+82
Button B: X+71, Y+16
Prize: X=6923, Y=8694

Button A: X+95, Y+73
Button B: X+13, Y+61
Prize: X=8506, Y=10464

Button A: X+78, Y+23
Button B: X+12, Y+59
Prize: X=4266, Y=4031

Button A: X+31, Y+87
Button B: X+71, Y+36
Prize: X=1330, Y=2100

Button A: X+15, Y+55
Button B: X+38, Y+13
Prize: X=16129, Y=12049

Button A: X+74, Y+18
Button B: X+14, Y+50
Prize: X=6380, Y=1904

Button A: X+36, Y+19
Button B: X+13, Y+31
Prize: X=13245, Y=15500

Button A: X+37, Y+16
Button B: X+18, Y+33
Prize: X=13068, Y=10692

Button A: X+56, Y+20
Button B: X+29, Y+62
Prize: X=5211, Y=3462

Button A: X+26, Y+59
Button B: X+45, Y+19
Prize: X=399, Y=7100

Button A: X+52, Y+14
Button B: X+25, Y+77
Prize: X=2490, Y=9454

Button A: X+42, Y+11
Button B: X+20, Y+36
Prize: X=9786, Y=1137

Button A: X+61, Y+14
Button B: X+49, Y+93
Prize: X=6529, Y=7303

Button A: X+27, Y+17
Button B: X+12, Y+41
Prize: X=11438, Y=1948

Button A: X+53, Y+11
Button B: X+54, Y+88
Prize: X=5465, Y=5665

Button A: X+15, Y+77
Button B: X+78, Y+18
Prize: X=4700, Y=10332

Button A: X+11, Y+59
Button B: X+48, Y+19
Prize: X=15210, Y=9409

Button A: X+16, Y+52
Button B: X+41, Y+17
Prize: X=8584, Y=11248

Button A: X+56, Y+13
Button B: X+29, Y+68
Prize: X=2453, Y=17869

Button A: X+35, Y+91
Button B: X+76, Y+17
Prize: X=3918, Y=6033

Button A: X+77, Y+57
Button B: X+21, Y+50
Prize: X=5152, Y=6708

Button A: X+91, Y+83
Button B: X+75, Y+12
Prize: X=12690, Y=8190

Button A: X+14, Y+55
Button B: X+73, Y+19
Prize: X=11601, Y=15256

Button A: X+50, Y+22
Button B: X+15, Y+90
Prize: X=5055, Y=5310

Button A: X+18, Y+89
Button B: X+68, Y+44
Prize: X=3436, Y=7638

Button A: X+39, Y+17
Button B: X+33, Y+62
Prize: X=3546, Y=5593

Button A: X+12, Y+37
Button B: X+69, Y+37
Prize: X=14282, Y=9906

Button A: X+63, Y+14
Button B: X+12, Y+59
Prize: X=13250, Y=17592

Button A: X+36, Y+13
Button B: X+11, Y+27
Prize: X=2067, Y=13051

Button A: X+20, Y+41
Button B: X+66, Y+12
Prize: X=5388, Y=3894

Button A: X+20, Y+56
Button B: X+67, Y+13
Prize: X=19450, Y=8218

Button A: X+78, Y+54
Button B: X+12, Y+79
Prize: X=1170, Y=6324

Button A: X+90, Y+18
Button B: X+85, Y+85
Prize: X=3190, Y=2542

Button A: X+41, Y+55
Button B: X+33, Y+11
Prize: X=4748, Y=13212

Button A: X+14, Y+32
Button B: X+71, Y+43
Prize: X=7996, Y=7508

Button A: X+21, Y+62
Button B: X+29, Y+12
Prize: X=9063, Y=14936

Button A: X+22, Y+62
Button B: X+55, Y+16
Prize: X=2156, Y=4130

Button A: X+59, Y+14
Button B: X+74, Y+83
Prize: X=12057, Y=8358

Button A: X+14, Y+37
Button B: X+76, Y+19
Prize: X=5898, Y=2312

Button A: X+91, Y+15
Button B: X+59, Y+69
Prize: X=3305, Y=1671

Button A: X+15, Y+37
Button B: X+49, Y+36
Prize: X=7184, Y=9695

Button A: X+20, Y+41
Button B: X+43, Y+18
Prize: X=463, Y=10089

Button A: X+94, Y+44
Button B: X+30, Y+82
Prize: X=7600, Y=5868

Button A: X+96, Y+13
Button B: X+32, Y+33
Prize: X=3424, Y=693

Button A: X+36, Y+68
Button B: X+79, Y+14
Prize: X=7294, Y=4312

Button A: X+20, Y+68
Button B: X+60, Y+23
Prize: X=8420, Y=15573

Button A: X+27, Y+74
Button B: X+62, Y+17
Prize: X=3968, Y=16866

Button A: X+73, Y+29
Button B: X+17, Y+45
Prize: X=3900, Y=5444

Button A: X+27, Y+52
Button B: X+36, Y+12
Prize: X=693, Y=876

Button A: X+37, Y+18
Button B: X+31, Y+51
Prize: X=6019, Y=18734

Button A: X+29, Y+76
Button B: X+42, Y+13
Prize: X=11614, Y=2906

Button A: X+14, Y+42
Button B: X+59, Y+35
Prize: X=18349, Y=11813

Button A: X+68, Y+48
Button B: X+39, Y+92
Prize: X=6801, Y=9636

Button A: X+25, Y+41
Button B: X+75, Y+35
Prize: X=7025, Y=4481

Button A: X+64, Y+37
Button B: X+11, Y+21
Prize: X=6001, Y=11072

Button A: X+58, Y+26
Button B: X+28, Y+70
Prize: X=6296, Y=7648

Button A: X+14, Y+53
Button B: X+81, Y+42
Prize: X=6345, Y=17070

Button A: X+14, Y+42
Button B: X+62, Y+18
Prize: X=2166, Y=18194

Button A: X+20, Y+75
Button B: X+48, Y+33
Prize: X=2368, Y=5058

Button A: X+25, Y+46
Button B: X+47, Y+19
Prize: X=13106, Y=17558

Button A: X+44, Y+16
Button B: X+45, Y+82
Prize: X=3277, Y=2570

Button A: X+86, Y+30
Button B: X+21, Y+91
Prize: X=4067, Y=8029

Button A: X+31, Y+80
Button B: X+85, Y+38
Prize: X=4335, Y=7560

Button A: X+61, Y+23
Button B: X+17, Y+47
Prize: X=4852, Y=2260

Button A: X+60, Y+15
Button B: X+21, Y+57
Prize: X=13505, Y=1985

Button A: X+36, Y+16
Button B: X+27, Y+43
Prize: X=16037, Y=9885

Button A: X+46, Y+13
Button B: X+21, Y+66
Prize: X=13369, Y=6940

Button A: X+14, Y+45
Button B: X+26, Y+16
Prize: X=9320, Y=13990

Button A: X+14, Y+26
Button B: X+92, Y+25
Prize: X=8294, Y=4318

Button A: X+53, Y+19
Button B: X+14, Y+26
Prize: X=16384, Y=10312

Button A: X+52, Y+75
Button B: X+63, Y+26
Prize: X=6045, Y=2816

Button A: X+15, Y+43
Button B: X+71, Y+23
Prize: X=16775, Y=9495

Button A: X+69, Y+33
Button B: X+40, Y+99
Prize: X=6091, Y=4191

Button A: X+99, Y+32
Button B: X+36, Y+51
Prize: X=7065, Y=5787

Button A: X+47, Y+70
Button B: X+60, Y+26
Prize: X=2586, Y=2014

Button A: X+91, Y+27
Button B: X+31, Y+68
Prize: X=8982, Y=8310

Button A: X+93, Y+45
Button B: X+32, Y+62
Prize: X=2799, Y=1773

Button A: X+19, Y+38
Button B: X+45, Y+13
Prize: X=13235, Y=531

Button A: X+25, Y+44
Button B: X+59, Y+27
Prize: X=5501, Y=2843

Button A: X+18, Y+36
Button B: X+68, Y+43
Prize: X=2396, Y=13667

Button A: X+67, Y+31
Button B: X+16, Y+61
Prize: X=9339, Y=13839

Button A: X+50, Y+18
Button B: X+59, Y+94
Prize: X=7668, Y=6544

Button A: X+76, Y+12
Button B: X+56, Y+66
Prize: X=7032, Y=6426

Button A: X+14, Y+37
Button B: X+65, Y+35
Prize: X=13499, Y=18892

Button A: X+17, Y+35
Button B: X+21, Y+12
Prize: X=4779, Y=16569

Button A: X+35, Y+41
Button B: X+83, Y+16
Prize: X=4487, Y=1276

Button A: X+39, Y+37
Button B: X+95, Y+15
Prize: X=6688, Y=2814

Button A: X+26, Y+55
Button B: X+35, Y+15
Prize: X=8743, Y=6110

Button A: X+42, Y+34
Button B: X+22, Y+75
Prize: X=958, Y=2720

Button A: X+67, Y+20
Button B: X+18, Y+60
Prize: X=14996, Y=6200

Button A: X+13, Y+49
Button B: X+68, Y+33
Prize: X=17288, Y=1213

Button A: X+13, Y+56
Button B: X+67, Y+31
Prize: X=4233, Y=3808

Button A: X+65, Y+17
Button B: X+13, Y+45
Prize: X=7790, Y=14958

Button A: X+42, Y+17
Button B: X+11, Y+44
Prize: X=18067, Y=5940

Button A: X+14, Y+40
Button B: X+49, Y+11
Prize: X=5873, Y=4009

Button A: X+70, Y+27
Button B: X+26, Y+42
Prize: X=9094, Y=6513

Button A: X+58, Y+31
Button B: X+16, Y+30
Prize: X=2210, Y=10445

Button A: X+38, Y+79
Button B: X+80, Y+54
Prize: X=10226, Y=10477

Button A: X+61, Y+51
Button B: X+34, Y+96
Prize: X=1296, Y=1962

Button A: X+66, Y+24
Button B: X+60, Y+83
Prize: X=5970, Y=7249

Button A: X+77, Y+33
Button B: X+15, Y+56
Prize: X=5348, Y=10861

Button A: X+97, Y+16
Button B: X+15, Y+75
Prize: X=3718, Y=2644

Button A: X+18, Y+66
Button B: X+57, Y+23
Prize: X=1331, Y=461

Button A: X+86, Y+19
Button B: X+11, Y+68
Prize: X=11493, Y=13480

Button A: X+58, Y+19
Button B: X+27, Y+65
Prize: X=4463, Y=3989

Button A: X+22, Y+99
Button B: X+58, Y+47
Prize: X=4658, Y=9619

Button A: X+41, Y+77
Button B: X+83, Y+15
Prize: X=3826, Y=5354

Button A: X+61, Y+87
Button B: X+88, Y+23
Prize: X=9185, Y=7872

Button A: X+29, Y+85
Button B: X+92, Y+70
Prize: X=1756, Y=3350

Button A: X+34, Y+14
Button B: X+34, Y+53
Prize: X=9560, Y=6383

Button A: X+58, Y+22
Button B: X+27, Y+57
Prize: X=3229, Y=11191

Button A: X+23, Y+11
Button B: X+24, Y+63
Prize: X=17926, Y=17857

Button A: X+13, Y+73
Button B: X+99, Y+22
Prize: X=979, Y=1760

Button A: X+85, Y+14
Button B: X+69, Y+68
Prize: X=3893, Y=1604

Button A: X+35, Y+70
Button B: X+48, Y+13
Prize: X=13198, Y=8123

Button A: X+19, Y+48
Button B: X+41, Y+17
Prize: X=12712, Y=4069

Button A: X+76, Y+29
Button B: X+74, Y+98
Prize: X=2248, Y=1974

Button A: X+18, Y+60
Button B: X+36, Y+33
Prize: X=4716, Y=7716

Button A: X+13, Y+38
Button B: X+35, Y+21
Prize: X=15360, Y=18553

Button A: X+30, Y+86
Button B: X+79, Y+47
Prize: X=8027, Y=8115

Button A: X+29, Y+11
Button B: X+43, Y+59
Prize: X=9067, Y=12323

Button A: X+37, Y+12
Button B: X+13, Y+37
Prize: X=5632, Y=200

Button A: X+83, Y+20
Button B: X+19, Y+72
Prize: X=6469, Y=6548

Button A: X+65, Y+20
Button B: X+21, Y+61
Prize: X=16475, Y=4040

Button A: X+44, Y+14
Button B: X+36, Y+78
Prize: X=5748, Y=16506

Button A: X+30, Y+11
Button B: X+20, Y+57
Prize: X=14450, Y=18684

Button A: X+67, Y+19
Button B: X+22, Y+72
Prize: X=13334, Y=18046

Button A: X+44, Y+46
Button B: X+85, Y+16
Prize: X=4389, Y=2184

Button A: X+61, Y+28
Button B: X+24, Y+62
Prize: X=9152, Y=946

Button A: X+62, Y+11
Button B: X+30, Y+63
Prize: X=6224, Y=5084

Button A: X+60, Y+33
Button B: X+15, Y+28
Prize: X=16985, Y=17028

Button A: X+17, Y+45
Button B: X+45, Y+26
Prize: X=12775, Y=255

Button A: X+32, Y+52
Button B: X+39, Y+14
Prize: X=16202, Y=7912

Button A: X+51, Y+11
Button B: X+90, Y+96
Prize: X=2856, Y=1918

Button A: X+55, Y+14
Button B: X+21, Y+59
Prize: X=7579, Y=18566

Button A: X+20, Y+45
Button B: X+47, Y+32
Prize: X=6786, Y=15691

Button A: X+83, Y+59
Button B: X+11, Y+83
Prize: X=1447, Y=4111

Button A: X+70, Y+23
Button B: X+16, Y+59
Prize: X=14404, Y=585

Button A: X+20, Y+50
Button B: X+55, Y+28
Prize: X=9160, Y=1204

Button A: X+68, Y+55
Button B: X+12, Y+42
Prize: X=3188, Y=3838

Button A: X+69, Y+80
Button B: X+11, Y+89
Prize: X=5158, Y=6819

Button A: X+29, Y+61
Button B: X+55, Y+13
Prize: X=3197, Y=8483

Button A: X+74, Y+25
Button B: X+11, Y+56
Prize: X=12703, Y=15991

Button A: X+60, Y+15
Button B: X+42, Y+95
Prize: X=4998, Y=9615

Button A: X+23, Y+32
Button B: X+87, Y+34
Prize: X=10409, Y=6300

Button A: X+11, Y+44
Button B: X+71, Y+24
Prize: X=13268, Y=17872

Button A: X+13, Y+60
Button B: X+63, Y+22
Prize: X=6522, Y=5106

Button A: X+87, Y+32
Button B: X+11, Y+66
Prize: X=4834, Y=4504

Button A: X+55, Y+22
Button B: X+21, Y+52
Prize: X=11757, Y=2214

Button A: X+28, Y+69
Button B: X+37, Y+13
Prize: X=3138, Y=17965

Button A: X+77, Y+13
Button B: X+19, Y+30
Prize: X=4100, Y=2782

Button A: X+29, Y+55
Button B: X+49, Y+27
Prize: X=17241, Y=12611

Button A: X+23, Y+13
Button B: X+23, Y+75
Prize: X=3427, Y=7703

Button A: X+43, Y+32
Button B: X+17, Y+55
Prize: X=907, Y=1649

Button A: X+20, Y+71
Button B: X+65, Y+39
Prize: X=6270, Y=5768

Button A: X+11, Y+58
Button B: X+81, Y+17
Prize: X=330, Y=17911

Button A: X+22, Y+31
Button B: X+54, Y+16
Prize: X=4142, Y=1570

Button A: X+25, Y+79
Button B: X+68, Y+12
Prize: X=19174, Y=458

Button A: X+15, Y+55
Button B: X+47, Y+18
Prize: X=6941, Y=19714

Button A: X+60, Y+30
Button B: X+48, Y+77
Prize: X=4812, Y=5533

Button A: X+71, Y+48
Button B: X+15, Y+61
Prize: X=2520, Y=3026

Button A: X+26, Y+12
Button B: X+18, Y+29
Prize: X=3506, Y=16969

Button A: X+16, Y+80
Button B: X+82, Y+39
Prize: X=8130, Y=10599

Button A: X+32, Y+79
Button B: X+89, Y+36
Prize: X=8654, Y=4095

Button A: X+28, Y+75
Button B: X+85, Y+60
Prize: X=2856, Y=2955

Button A: X+91, Y+51
Button B: X+20, Y+68
Prize: X=9032, Y=6936

Button A: X+36, Y+80
Button B: X+43, Y+12
Prize: X=14547, Y=14252

Button A: X+95, Y+16
Button B: X+25, Y+23
Prize: X=2855, Y=1627

Button A: X+46, Y+23
Button B: X+49, Y+93
Prize: X=2124, Y=2295

Button A: X+27, Y+84
Button B: X+53, Y+11
Prize: X=10279, Y=17893

Button A: X+19, Y+55
Button B: X+42, Y+23
Prize: X=12761, Y=2299

Button A: X+50, Y+66
Button B: X+75, Y+30
Prize: X=1550, Y=1770

Button A: X+27, Y+95
Button B: X+85, Y+17
Prize: X=3986, Y=3306

Button A: X+95, Y+49
Button B: X+19, Y+85
Prize: X=8056, Y=7088

Button A: X+49, Y+68
Button B: X+88, Y+34
Prize: X=3014, Y=2244

Button A: X+46, Y+30
Button B: X+17, Y+41
Prize: X=13436, Y=18172

Button A: X+69, Y+28
Button B: X+23, Y+60
Prize: X=5566, Y=3576

Button A: X+44, Y+54
Button B: X+12, Y+82
Prize: X=4236, Y=11186

Button A: X+11, Y+64
Button B: X+79, Y+24
Prize: X=13813, Y=11296

Button A: X+68, Y+68
Button B: X+13, Y+93
Prize: X=1895, Y=3095

Button A: X+45, Y+17
Button B: X+44, Y+77
Prize: X=284, Y=19673

Button A: X+68, Y+14
Button B: X+67, Y+78
Prize: X=8070, Y=7440

Button A: X+73, Y+11
Button B: X+21, Y+95
Prize: X=1919, Y=7085

Button A: X+26, Y+72
Button B: X+97, Y+35
Prize: X=2263, Y=3697

Button A: X+33, Y+13
Button B: X+36, Y+51
Prize: X=17687, Y=14407

Button A: X+62, Y+35
Button B: X+31, Y+60
Prize: X=15832, Y=16295

Button A: X+62, Y+29
Button B: X+36, Y+89
Prize: X=8392, Y=10059

Button A: X+55, Y+28
Button B: X+48, Y+91
Prize: X=6081, Y=8554

Button A: X+13, Y+34
Button B: X+84, Y+42
Prize: X=2351, Y=3128

Button A: X+14, Y+31
Button B: X+47, Y+30
Prize: X=13965, Y=6961

Button A: X+88, Y+74
Button B: X+15, Y+63
Prize: X=7553, Y=10735

Button A: X+70, Y+20
Button B: X+55, Y+77
Prize: X=11750, Y=9118

Button A: X+43, Y+61
Button B: X+35, Y+15
Prize: X=5417, Y=8049

Button A: X+46, Y+25
Button B: X+28, Y+52
Prize: X=4390, Y=3379

Button A: X+25, Y+57
Button B: X+50, Y+13
Prize: X=2025, Y=3102

Button A: X+62, Y+13
Button B: X+24, Y+64
Prize: X=1698, Y=1023

Button A: X+16, Y+71
Button B: X+61, Y+19
Prize: X=5497, Y=4908

Button A: X+23, Y+97
Button B: X+35, Y+20
Prize: X=3969, Y=10486

Button A: X+96, Y+24
Button B: X+71, Y+71
Prize: X=5168, Y=2144

Button A: X+44, Y+73
Button B: X+33, Y+11
Prize: X=15511, Y=6412

Button A: X+42, Y+23
Button B: X+17, Y+40
Prize: X=1178, Y=2018

Button A: X+44, Y+95
Button B: X+47, Y+32
Prize: X=7163, Y=9560
"""

from dataclasses import dataclass
import re

@dataclass
class Button:
    xDelta: int
    yDelta: int
    cost: int

@dataclass(frozen=True, order=True)
class Location:
    x: int
    y: int

@dataclass
class Machine:
    A: Button
    B: Button
    prizeLocation: Location

def parse(input: str):
    lines = input.split("\n")
    button_regex = r'Button (A|B): X\+(?P<x>\d+), Y\+(?P<y>\d+)'
    prize_regex = r'Prize: X=(?P<x>\d+), Y=(?P<y>\d+)'
    machines = list()
    for i in range(0, len(lines), 4):
        matched = re.match(button_regex, lines[i])
        a_button = Button(int(matched.group('x')), int(matched.group('y')), 3)
        matched = re.match(button_regex, lines[i+1])
        b_button = Button(int(matched.group('x')), int(matched.group('y')), 1)
        matched = re.match(prize_regex, lines[i+2])
        prizeLocation = Location(int(matched.group('x')) + 10000000000000, int(matched.group('y')) + 10000000000000)
        machines.append(Machine(a_button, b_button, prizeLocation))
    return machines
    
def solveMachine(machine: Machine) -> int:
    # Solving the following system of equations:
    # let n = number times A pressed, m = number times B pressed, c = prize location
    # ax * n + bx * m = cx, ay * n + by * m = cy
    # Something something algebra: https://chatgpt.com/share/675beb0b-7f58-800c-a25c-0d86b686d68f

    n = int((machine.prizeLocation.x * machine.B.yDelta - machine.prizeLocation.y * machine.B.xDelta) / (machine.A.xDelta * machine.B.yDelta - machine.A.yDelta * machine.B.xDelta))
    m = int((machine.A.xDelta * machine.prizeLocation.y - machine.A.yDelta * machine.prizeLocation.x) / (machine.A.xDelta * machine.B.yDelta - machine.A.yDelta * machine.B.xDelta))
    if Location(n * machine.A.xDelta + m * machine.B.xDelta, n * machine.A.yDelta + m * machine.B.yDelta) == machine.prizeLocation:
        return 3 * n + m
    return 0

def solveAllMachines(machines: list[Machine]) -> int:
    return sum([solveMachine(machine) for machine in machines])


print(solveAllMachines(parse(input)))
    



