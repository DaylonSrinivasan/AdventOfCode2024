...
def parse(input: str):
    ...
    for i in range(0, len(lines), 4):
        ...
        prizeLocation = Location(int(matched.group('x')), int(matched.group('y')))
        machines.append(Machine(a_button, b_button, prizeLocation))
    return machines
    
def solveMachine(machine: Machine) -> int:
    priorityQueue: list[tuple[int, Location]] = list()
    heapq.heappush(priorityQueue, (0, Location(0, 0)))
    visited: set[Location] = set()
    while priorityQueue:
        cost, location = heapq.heappop(priorityQueue)
        if location in visited or location.x > machine.prizeLocation.x or location.y > machine.prizeLocation.y:
            continue
        if location == machine.prizeLocation:
            return cost
        else:
            visited.add(location)
            heapq.heappush(priorityQueue, (cost + machine.A.cost, Location(location.x + machine.A.xDelta, location.y + machine.A.yDelta)))
            heapq.heappush(priorityQueue, (cost + machine.B.cost, Location(location.x + machine.B.xDelta, location.y + machine.B.yDelta)))
