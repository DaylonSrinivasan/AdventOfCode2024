...
class TowelArranger:
    ...
    def num_arrangements(self) -> int:
        return sum([self.num_arrangements_for_pattern(pattern, 0) for pattern in self.patterns])

    @lru_cache(maxsize=5000)
    def num_arrangements_for_pattern(self, pattern: str, idx: int) -> int:
        if idx == len(pattern):
            return 1
        ans = 0
        for towel in self.towels:
            if pattern[idx:].startswith(towel):
                ans += self.num_arrangements_for_pattern(pattern, idx+len(towel))
        return ans

### Run program ###
print(TowelArranger(towelInput, patternInput).num_arrangements())