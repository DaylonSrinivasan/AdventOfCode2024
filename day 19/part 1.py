...
class TowelArranger:
    def __init__(self, towelInput: str, patternInput: str):
        self.towels = [towel.strip() for towel in towelInput.split(",")]
        self.patterns = patternInput.split("\n")
    
    def num_possible_patterns(self) -> int:
        return len([True for pattern in self.patterns if self.is_possible(pattern, 0)])

    def is_possible(self, pattern: str, idx: int) -> bool:
        if idx == len(pattern):
            return True
        for towel in self.towels:
            if pattern[idx:].startswith(towel) and self.is_possible(pattern, idx+len(towel)):
                return True
        return False

### Run program ###
print(TowelArranger(towelInput, patternInput).num_possible_patterns())